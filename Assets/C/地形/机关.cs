...
public class 感应 : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision == Player3.I.蹲BOX)
            ...
            玩家进入了检测范围 = true;
        //if (Deb)Debug.LogError("OnTriggerEnter2D" + collision.gameObject.name);
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision == Player3.I.蹲BOX)
            ...
            玩家进入了检测范围 = false;
        //if (Deb) Debug.LogError("OnTriggerExit2D" + collision.gameObject.name);
...
public partial class 机关 : 交互_Base
{
    ...
    {
        var B = 特效_pool_2.I.GetPool(a.transform.position, T_N.特效大爆炸).Speed_Lv = Player3.Public_Const_Speed;
    }
    public void 销毁触发_()
    {
        销毁触发?.Invoke();
    ...
    void 真提示(string s)
    {
        ...
        if (适应 == null)
        {
            Debu.LogError("A  适应.开关(true);  适应.开关(true);BBBBBBBBBBBBBBBBBBBBBBBBBBB");
            Debug.LogError("子物体没有适应文字组件");
        }
        else
        {
            Debug.LogError("设置了");
            Debu.LogError("A  适应.开关(true);  适应.开关(true);AAAAAAAAAAAAAAAAAAAA");
            适应.开关(true);
            适应.SetText(s);
        }
    }

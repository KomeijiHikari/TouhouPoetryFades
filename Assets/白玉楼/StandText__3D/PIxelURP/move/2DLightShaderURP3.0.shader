Shader "ORIS Shaders/my2D Light Shader URP"
{
    Properties
    {
        ...
        [HideInInspector] _Z("Z Texture", 2D) = "white" {}
                ...
                [HideInInspector] _NorTexx("Normal Texture", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Blur(float2 uv, float f = 1)
            {
                ...
                for (int y = -i; y <= i; y++)
                {
                    ...
                    {
                        half2 Of = half2(x, y);
                        half L = tex2D(_DepthTex, center + Of * _DepthTex_TexelSize).x;
                        float4 sampleColor = OC(_MainTex, uv + Of * texelSize);
                        Color += sampleColor * L;
                        Valid += L;
                    }
                }
                Valid = max(Valid, 0.0001);
                Color /= Valid;
                return Color;
            }

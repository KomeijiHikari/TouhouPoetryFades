...
public class Fly_Ground : MonoBehaviour, I_Speed_Change, I_攻击, I_ReturnPool, I_Dead
{
    ...
    private void 箭头()
    {
        ...
        if (Debul) Debug.LogError(" 箭头()     箭头()" + dirEnum);
      var Or=  bc.bounds.九个点(dirEnum);
        Collider2s= Physics2D.OverlapCircleAll(Or, 0.3f, Currrtten) ;
        for (int i = 0; i < Collider2s.Length; i++)
        {
            var a= Collider2s[i];
            if (a == null) continue;
            if (a == bc) continue;
            if (a.CompareTag(Initialize.Player))
            {
                ///默认有伤害
                ///方向相同不造成伤害
                ///
                Debug.LogError(方向);
                if (!方向和速度(方向, Player3.I.Velocity))
                {
                    引线爆炸(a); 
                }
          ...
          if (a.gameObject.TryGetComponent<Fly_Ground>(out var f)) continue;
    ...
    public void 假死了(bool 假死不)
    {
        ...
        bool 方向和速度(Vector2 direction, Vector2 compareVector)
    {
        // 严格检查：X和Y必须是-1、0、1的整数
        ...
        if (Mathf.Abs(x) > 1 || Mathf.Abs(y) > 1)
          ...
          // 使用严格检查后的值
          Vector2 strictDirection = new Vector2(x, y);
        ...
        if (不合理)
        {
            Debug_.LogError("不河理"+ gameObject.name+transform.position,this);
            return false;
        }
        ...
        if(x!=0)
        {
            ...
            {
                return Mathf.Sign(x)==Mathf.Sign(compareVector.x);
            }
        }
        else
        {
            ...
            {
                return Mathf.Sign(y) == Mathf.Sign(compareVector.y);
            }
        }
        //// 比较逻辑
        //if (x != 0)
        //{
        //    return  
        //    //return !Mathf.Approximately(compareVector.x, 0f) && Mathf.Sign(x) == Mathf.Sign(compareVector.x);
        //}
        //else
        //{
        //    return Mathf.Approximately(compareVector.y, 0f) ? false :
        //           Mathf.Sign(y) == Mathf.Sign(compareVector.y);
        //}
    }
}

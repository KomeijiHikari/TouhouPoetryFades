...
Shader "ORIS Shaders/2D Light Shader URP"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
						...
						, half ase_vface : VFACE ) : SV_Target
			{
				...
				float2 uv_SpriteTexture = IN.ase_texcoord7.xy * _SpriteTexture_ST.xy + _SpriteTexture_ST.zw;
				...
				float3 Albedo = pow( (( _BlackWhite )?( appendResult30 ):( appendResult9 )) , temp_cast_0 ).xyz;
				...
				#endif
				//appendResult9合成后的颜色 没有W
				//IN.clipPos确实是对的 IN.clipPos.y/1080可以
				return color;
			}
		...
		Pass
		{
						 ...
						 ) : SV_TARGET
			{
				...
				float2 uv_SpriteTexture = IN.ase_texcoord2.xy * _SpriteTexture_ST.xy + _SpriteTexture_ST.zw;
				float4 tex2DNode1 = tex2D( _SpriteTexture, uv_SpriteTexture );
